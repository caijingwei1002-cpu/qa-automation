...
当前计划包含 182 个核心学习日，覆盖 UI、API、性能、测试工程化、可靠性和作品集整理。计划由 [tools/build_daily_plan.py](tools/build_daily_plan.py) 生成，`daily-plan.json` 和 `DAILY-PLAN.md` 是生成结果，不应手工维护其中的单日路径。

## 每日完成标准

每个学习日至少留下：

1. 一个新增或改进的可运行测试脚本或工程改动；
2. 一次真实执行结果，失败时记录根因或阻塞条件；
3. 一份学习记录和证据路径；
4. 一次有意义的 Git 提交。
